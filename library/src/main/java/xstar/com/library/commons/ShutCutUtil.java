...
public class ShutCutUtil {
    ...
    /**
     * 添加快捷方式到桌面，添加快捷方式需要添加用户权限 <uses-permission android:name=
    ...
    /**
     * 删除桌面上的快捷方式，需要添加权限 <uses-permission android:name=
     ...
     * 其中一张表就是favorites。这个db文件一般放在data/data/com.android.launcher(launcher2)
     * 文件的databases下 但是对于不同的rom会放在不同的地方
